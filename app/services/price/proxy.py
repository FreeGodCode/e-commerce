...
# @File name: proxy.py 
# @IDE: PyCharm
# @Create time: 1/26/21 5:35 PM
# @Description: 代理
import datetime

from app import db
from app.config.enum import COUPON_APPLY, COUPON_SCOPE
from app.models.coupon.coupon import Coupon
from app.services.price.coupon import CouponList, CouponConflict
...
class PriceEval(DocProxy):
    def __init__(self, obj):
        ...
        if getattr(obj, 'is_paid', False):
            for coupon in Coupon.objects(code__in=self.coupon_codes):
                self.add_coupon(coupon)
        else:
            for coupon in Coupon.objects((db.Q(code__in=self.coupon_codes) | db.Q(apply=COUPON_APPLY.AUTO)) & db.Q(
                    effective_date__lt=datetime.datetime.utcnow(), expire_date__gt=datetime.datetime.utcnow())):
    ...
    def can_add_coupon(self, coupon):
        ...
        :return:
        """
        return (coupon.apply in (COUPON_APPLY.AUTO, COUPON_APPLY.BY_CODE) or self._obj.customer.wallet.has_coupon(
            coupon))
    ...
    def apply_base_price(self):
        for attr in ('amount_usd', 'amount', 'cn_shipping', 'final', 'forex', 'logistic_provider'):
