# -*- coding: utf-8  -*-
# @Author: ty
# @File name: partner.py 
# @IDE: PyCharm
# @Create time: 1/14/21 3:56 PM
# @Description:
import hashlib
import math
...
__all__ = ['WeightPrice', 'LogisticProvider', 'ChannelProvider']
...
class WeightPrice(object):
    ...
    def get_shipping(self, weight):
        ...
        return round(self.init_price + math.ceil(
            (float(weight) - self.init_weight) / self.continued_weight) * self.continued_price, ndigits=2)
...
class Partner(db.Document):
    ...
    def create(cls, name, password, description):
        partner = cls(name=name, auth=hashlib.sha1(name).hexdigest(), description=description)
    ...
    @classmethod
    def authentication(cls, auth, password):
    ...
    def get_shipping(self, weight):
        provider = LogisticProvider.objects.get(name=self.name)
...
class ChannelProvider(db.Document):
    ...
    def to_json(self):
        return dict(name=self.name, display_name=self.display_name, service_intro=self.service_intro,
                    desc=self.description)
