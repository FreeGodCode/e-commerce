...
# @File name: price.py 
# @IDE: PyCharm
# @Create time: 1/17/21 4:49 PM
# @Description:
import datetime
from math import ceil

from app import db
from app.config.enum import CURRENCY

__all__ = ['Price', 'PriceHistory', 'ForexRate']


class Price(db.EmbeddedDocument):
    """"""
    price = db.FloatField(required=True, min_value=0)
    record_date = db.DateTimeField(default=datetime.datetime.utcnow, required=True)
...
class ForexRate(db.Document):
    ...
    def get(cls, currency=CURRENCY.USD):
        ...
        start = (datetime.datetime.utcnow() - datetime.timedelta(days=3)).replace(minute=0, second=0, microsecond=0)
    ...
    def put(cls, rate, d=None, currency=CURRENCY.USD):
        if d is None:
            ...
            cls.objects(currency=currency, modified=d).update_one(set__rate=rate, upsert=True)
